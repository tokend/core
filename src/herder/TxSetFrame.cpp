...
static bool
SaltSorter(TransactionFramePtr const& tx1, TransactionFramePtr const& tx2)
{
...
TxSetFrame::sortForApply()
{
    ...
    // sort all the txs by salt
    std::sort(retList.begin(), retList.end(), SaltSorter);
...
struct SurgeSorter
{
    map<AccountID, int64>& mAccountFeeMap;
    SurgeSorter(map<AccountID, int64>& afm) : mAccountFeeMap(afm)
    {
    }
    ...
    operator()(TransactionFramePtr const& tx1, TransactionFramePtr const& tx2)
    {
        if (tx1->getSourceID() == tx2->getSourceID())
            return tx1->getSalt() < tx2->getSalt();
        auto fee1 = mAccountFeeMap[tx1->getSourceID()];
        auto fee2 = mAccountFeeMap[tx2->getSourceID()];
        if (fee1 == fee2)
...
TxSetFrame::surgePricingFilter(LedgerManager const& lm)
{
    ...
    { // surge pricing in effect!
        ...
        map<AccountID, int64> accountFeeMap;
        for (auto& tx : mTransactions)
        {
			auto r = tx->getPaidFee();
            int64 now = accountFeeMap[tx->getSourceID()];
            if (now == 0)
