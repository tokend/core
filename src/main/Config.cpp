...
operationalID(PubKeyUtils::fromStrKey("GABAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABVCX"))
{
    ...
    // non configurable
    FORCE_SCP = false;
    LEDGER_PROTOCOL_VERSION = static_cast<int32_t>(LedgerVersion::ADD_DEFAULT_ISSUANCE_TASKS);
    ...
    try
    {
        ...
        for (auto& item : g)
        {
            ...
            else if (item.first == "BASE_EXCHANGE_NAME")
            {
                if (!item.second->as<std::string>())
                {
                    throw std::invalid_argument("invalid BASE_EXCHANGE_NAME");
                }
            ...
            else if (item.first == "TX_EXPIRATION_PERIOD")
            {
                ...
                TX_EXPIRATION_PERIOD = item.second->as<int64_t>()->value();
            }
