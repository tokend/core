...
namespace stellar
{
    ...
    static const char* atomicSwapBidColumnSelector = "SELECT bid_id, owner_id, "
                                                     ...
                                                     "fee, percent_fee, price, details, created_at, "
                                           ...
                                           Database &db, LedgerDelta *delta)
    {
        ...
        loadAtomicSwapBids(prep, [&retBid](LedgerEntry const& atomicSwapBid) {
    ...
    AtomicSwapBidHelper::loadAllAtomicSwapBids(Database &db)
    {
        ...
        auto timer = db.getSelectTimer("atomic-swap-bid");
        loadAtomicSwapBids(prep, [&retBids](LedgerEntry const& entry) {
    ...
    void
    AtomicSwapBidHelper::storeUpdateHelper(LedgerDelta &delta, Database &db, bool insert, LedgerEntry const &entry)
    {
        ...
        if (insert)
        {
            sql = "INSERT INTO atomic_swap_bid (bid_id, owner_id, "
                  "base_asset_code, quote_asset_code, base_balance_id, "
                  "base_amount, fee, percent_fee, price, "
                  "details, created_at, lastmodified, version) "
                  "VALUES "
                  "(:id, :oid, :bac, :qac, :bbi, :ba, :f, :pf, :p, :d, :ca, :lm, :v)";
        }
        else
        {
            sql = "UPDATE atomic_swap_bid "
                  "SET base_asset_code = :bac, quote_asset_code = :qac, "
                  "base_balance_id = :bbi, base_amount = :ba, "
                  "fee = :f, percent_fee = :pf, price = :p, details = :d, "
    ...
    void
    AtomicSwapBidHelper::loadAtomicSwapBids(StatementContext &prep,
                                            std::function<void(LedgerEntry const &)> atomicSwapBidProcessor)
    {
        ...
        st.exchange(into(be.quoteAsset));
        st.exchange(into(be.baseBalance));
        st.exchange(into(be.baseAmount));
        st.exchange(into(be.fee));
        st.exchange(into(be.percentFee));
        st.exchange(into(be.price));
