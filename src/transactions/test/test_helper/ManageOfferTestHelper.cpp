...
void
ManageOfferTestHelper::ensureDeleteSuccess(
    Account& source, ManageOfferOp op, const ManageOfferSuccessResult success,
...
ManageOfferTestHelper::ManageOfferTestHelper(
    ...
    const bool isBuy, const int64_t fee)
{
    ...
    op.body.manageOfferOp().orderBookID =
        ManageOfferOpFrame::SECONDARY_MARKET_ORDER_BOOK_ID;
...
ManageOfferResult
ManageOfferTestHelper::applyManageOffer(Account& source,
                                        ManageOfferOp& manageOfferOp,
                                        ManageOfferResultCode expectedResult,
                                        OperationResultCode expectedOpCode)
{
    ...
    {
        ManageOfferTestHelper::ensureCreateSuccess(
            source, manageOfferOp, manageOfferResult.success(), stateBeforeOp);
        return manageOfferResult;
    }
...
TransactionFramePtr
ManageOfferTestHelper::createManageOfferTx(Account& source,
                                           ManageOfferOp& manageOfferOp)
{
    Operation op;
    op.body.type(OperationType::MANAGE_OFFER);
...
ManageOfferOp
ManageOfferTestHelper::manageOfferOp(const uint64_t offerID,
                                     BalanceID const& baseBalance,
                                     BalanceID const& quoteBalance,
                                     const int64_t amount, const int64_t price,
                                     const bool isBuy, const int64_t fee)
{
    ManageOfferOp op;
    op.amount = amount;
    ...
    op.fee = fee;
    op.orderBookID = ManageOfferOpFrame::SECONDARY_MARKET_ORDER_BOOK_ID;
