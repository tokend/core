...
TEST_CASE("Sale Requests", "[tx][sale_requests]")
{
    ...
    SECTION("Create sale request with tasks and cancel")
    {
        ...
        SECTION("Update sale request")
        {
            ...
            auto requestAfterUpdate = requestHelper.loadRequest(requestID);
    ...
    {
        auto sellerFeeFrame = FeeFrame::create(FeeType::OFFER_FEE, 0, int64_t(
            2 * ONE), quoteAsset, &syndicatePubKey, precision);
        ...
        auto ownBalance = balanceHelper.loadBalance(syndicatePubKey, baseAsset);

        IssuanceRequestHelper(testManager).applyCreateIssuanceRequest(syndicate, baseAsset,
                                                                      10 * ONE, ownBalance->getBalanceID(),
    ...
    SECTION("Simple happy path for test fee")
    {
        ...
        auto commissionBalance = balanceHelper.loadBalance(app.getAdminID(), quoteAsset);
    ...
    SECTION("Review SaleCreationRequest with tasks")
    {
        ...
        SECTION("Max issuance or preissued amount is less then hard cap")
        {
            ...
            saleRequest = saleRequestHelper.createSaleRequest(asset, quoteAsset, currentTime,
                                                              currentTime + 1000, softCap, hardCap, "{}",
                                                              {saleRequestHelper.createSaleQuoteAsset(quoteAsset, price)},
                                                              900 * ONE);
    ...
    SECTION("Try to steal token by creating sale for stranger asset")
    {
        ...
        auto request = requestHelper.loadRequest(ownerAssetRequestID);
        REQUIRE(request);
