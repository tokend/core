...
class MockAccountSpecificRuleHelper : public AccountSpecificRuleHelper
{
public:
    ...
    MOCK_METHOD1(loadRule,
            AccountSpecificRuleFrame::pointer(uint64_t id));
    MOCK_METHOD3(loadRule,
            AccountSpecificRuleFrame::pointer(LedgerKey const& ledgerKey, AccountID const* accountID, bool exact));
    MOCK_METHOD1(mustLoadRule,
            AccountSpecificRuleFrame::pointer(uint64_t id));
    MOCK_METHOD1(exists, bool(uint64_t id));
    MOCK_METHOD1(deleteRulesForEntry, void(LedgerKey const& key));
};
