...
TEST_CASE("Sale", "[tx][sale]")
{
    ...
    ReviewUpdateSaleDetailsRequestTestHelper
        reviewUpdateSaleDetailsRequestTestHelper(testManager);
    ...
    longstring saleCreateKey =
        ManageKeyValueOpFrame::makeSaleCreateTasksKey("*");
    manageKeyValueHelper.setKey(saleCreateKey)->setUi32Value(0);
    manageKeyValueHelper.doApply(testManager->getApp(), ManageKVAction::PUT,
                                 true);
    longstring saleUpdateKey =
        ManageKeyValueOpFrame::makeSaleUpdateTasksKey("*");
    manageKeyValueHelper.setKey(saleUpdateKey)->setUi32Value(0);
    manageKeyValueHelper.doApply(testManager->getApp(), ManageKVAction::PUT,
                                 true);

    auto assetCreationRequest = assetTestHelper.createAssetCreationRequest(
        quoteAsset, root.key.getPublicKey(), "{}", maxNonDividedAmount,
        uint32_t(AssetPolicy::BASE_ASSET), nullptr, 0,
        testSet.trailingDigitsCount);
    ...
    auto ruleEntry = manageAccountRuleTestHelper.createAccountRuleEntry(
        0, baseAssetResource, AccountRuleAction::ANY, false);
    auto baseAssetOwnerRuleID =
    ...
    auto syndicateRoleID =
        manageAccountRoleTestHelper.applyTx(root, createSyndicateRoleOp)
            .success()
            .roleID;

    // basic account builder
    auto createAccountBuilder = CreateAccountTestBuilder().setSource(root);

    auto syndicate = Account{SecretKey::random(), 0};
    auto syndicatePubKey = syndicate.key.getPublicKey();
    createAccountTestHelper.applyTx(
        createAccountBuilder.setToPublicKey(syndicatePubKey)
            .addBasicSigner()
            .setRoleID(1));
    // TODO: for now we need to keep maxIssuance = preIssuance to allow sale
    // creation
    const uint64_t maxIssuanceAmount = 6000 * ONE;
    const uint64_t preIssuedAmount = maxIssuanceAmount;
    assetCreationRequest = assetTestHelper.createAssetCreationRequest(
        baseAsset, syndicate.key.getPublicKey(), "{}", maxIssuanceAmount, 0,
        nullptr, preIssuedAmount, testSet.trailingDigitsCount);
    assetTestHelper.createApproveRequest(root, syndicate, assetCreationRequest);
    const uint64_t price = 2 * ONE;
    auto hardCap = static_cast<const uint64_t>(
        bigDivide(preIssuedAmount / 2, price, ONE, ROUND_DOWN));
    ...
    auto saleRequest = SaleRequestHelper::createSaleRequest(
        baseAsset, quoteAsset, currentTime, endTime, softCap, hardCap, "{}",
    ...
    SECTION("Happy path")
    {
        ...
        // TODO: use set fees
        auto participantsFeeFrame =
            FeeFrame::create(FeeType::INVEST_FEE, 0, int64_t(1 * ONE),
                             quoteAsset, nullptr, precision);
        LedgerDeltaImpl delta(
            testManager->getLedgerManager().getCurrentLedgerHeader(), db);
        EntryHelperProvider::storeAddEntry(delta, db,
                                           participantsFeeFrame->mEntry);
        auto fee = setFeesTestHelper.createFeeEntry(
            FeeType::CAPITAL_DEPLOYMENT_FEE, quoteAsset, 0, 1 * ONE, nullptr,
        ...
        participantsFeeFrame->calculatePercentFee(hardCap, quotePreIssued,
                                                  ROUND_UP, 1);
        quotePreIssued += hardCap + ONE;
        IssuanceRequestHelper(testManager)
            .authorizePreIssuedAmount(root, root.key, quoteAsset,
                                      quotePreIssued, root);

        saleRequestHelper.createApprovedSale(root, syndicate, saleRequest);

        auto sales = SaleHelper::Instance()->loadSalesForOwner(
            syndicate.key.getPublicKey(), testManager->getDB());
        REQUIRE(sales.size() == 1);
        const auto saleID = sales[0]->getID();
        SECTION("Create second sale for the same base asset and close both")
        {
            // autoreview
            auto createSecondSaleRequestResult =
                saleRequestHelper.applyCreateSaleRequest(syndicate, 0,
                                                         saleRequest);
            sales = SaleHelper::Instance()->loadSalesForOwner(
                syndicate.key.getPublicKey(), testManager->getDB());
            ...
            uint64_t feeToPay(0);
            participantsFeeFrame->calculatePercentFee(quoteAssetAmount,
                                                      feeToPay, ROUND_UP, 1);

            // first sale reached hard cap
            for (auto i = 0; i < numberOfParticipants; i++)
            {
                participationHelper.addNewParticipant(
                    root, saleID, baseAsset, quoteAsset, quoteAssetAmount,
                    price, feeToPay);
                if (i < numberOfParticipants - 1)
                {
                    CheckSaleStateHelper(testManager)
                        .applyCheckSaleStateTx(
                            root, saleID, CheckSaleStateResultCode::NOT_READY);
                }
            }

            CheckSaleStateHelper(testManager)
                .applyCheckSaleStateTx(root, saleID);

            // second sale reached hard cap
            for (auto i = 0; i < numberOfParticipants; i++)
            {
                participationHelper.addNewParticipant(
                    root, secondSaleID, baseAsset, quoteAsset, quoteAssetAmount,
                    price, feeToPay);
                if (i < numberOfParticipants - 1)
                {
                    CheckSaleStateHelper(testManager)
                        .applyCheckSaleStateTx(
                            root, secondSaleID,
                            CheckSaleStateResultCode::NOT_READY);
                }
            }
            ...
            CheckSaleStateHelper(testManager)
                .applyCheckSaleStateTx(root, secondSaleID);
        }
        SECTION("Create several sales for the same base asset")
        {
            ...
            auto createThirdSaleRequestResult =
                saleRequestHelper.applyCreateSaleRequest(
                    syndicate, 0, saleRequest, nullptr,
                    CreateSaleCreationRequestResultCode::
                        INSUFFICIENT_MAX_ISSUANCE);

            sales = SaleHelper::Instance()->loadSalesForOwner(
                syndicate.key.getPublicKey(), testManager->getDB());
            REQUIRE(sales.size() == 2);

            // cancel one of two sales to check that right base amount was
            // unlocked
            auto data = manageSaleTestHelper.createDataForAction(
        ...
        SECTION("Try to cancel sale offer as regular one")
        {
            ...
            participantsFeeFrame->calculatePercentFee(quoteAssetAmount,
                                                      feeToPay, ROUND_UP, 1);
            const auto offerID = participationHelper.addNewParticipant(
                root, account, saleID, baseAsset, quoteAsset, quoteAssetAmount,
                price, feeToPay);
            ...
            auto manageOfferOp = OfferManager::buildManageOfferOp(
                offerEntry.baseBalance, offerEntry.quoteBalance, true, 0, price,
                0, offerEntry.offerID, 0);
            ParticipateInSaleTestHelper(testManager)
                .applyManageOffer(account, manageOfferOp,
                                  ManageOfferResultCode::NOT_FOUND);
        ...
        SECTION("Reached soft cap")
        {
            ...
            uint64_t feeToPay(0);
            participantsFeeFrame->calculatePercentFee(quoteAmount, feeToPay,
                                                      ROUND_UP, 1);
            const int64_t timeStep =
                (endTime - currentTime) / numberOfParticipants;
            for (int i = 0; i < numberOfParticipants - 1; i++)
            {
                participationHelper.addNewParticipant(root, saleID, baseAsset,
                                                      quoteAsset, quoteAmount,
                                                      price, feeToPay);
                testManager->advanceToTime(
                    testManager->getLedgerManager().getCloseTime() + timeStep);
                checkStateHelper.applyCheckSaleStateTx(
                    root, saleID, CheckSaleStateResultCode::NOT_READY);
            }
            // sale is still active
            participantsFeeFrame->calculatePercentFee(2 * quoteAmount, feeToPay,
                                                      ROUND_UP, 1);
            participationHelper.addNewParticipant(root, saleID, baseAsset,
                                                  quoteAsset, 2 * quoteAmount,
                                                  price, feeToPay);
            testManager->advanceToTime(endTime + 1);
        ...
        SECTION("Canceled")
        {
            ...
            testManager->advanceToTime(endTime + 1);
            auto checkRes = checkStateHelper.applyCheckSaleStateTx(
                root, saleID, CheckSaleStateResultCode::SUCCESS);
            REQUIRE(checkRes.success().effect.effect() ==
        ...
        SECTION("Manage sale")
        {
            SECTION("Update sale details")
            {
                ...
                auto manageSaleData = manageSaleTestHelper.createDataForAction(
                    ManageSaleAction::CREATE_UPDATE_DETAILS_REQUEST, &tasks,
                    &requestID, &newDetails);
                auto manageSaleResult = manageSaleTestHelper.applyManageSaleTx(
                    syndicate, saleID, manageSaleData);

                SECTION("Request already exists")
                {
                    manageSaleTestHelper.applyManageSaleTx(
                        syndicate, saleID, manageSaleData,
                        ManageSaleResultCode::
                            UPDATE_DETAILS_REQUEST_ALREADY_EXISTS);
                }

                SECTION("Sale not found")
                {
                    manageSaleTestHelper.applyManageSaleTx(
                        syndicate, 42, manageSaleData,
            ...
            SECTION("Tasks permissions")
            {
                ...
                auto txRuleEntry =
                    manageAccountRuleTestHelper.createAccountRuleEntry(
                        0, txResource, AccountRuleAction::SEND, false);
                auto txRuleId = manageAccountRuleTestHelper
                                    .applyTx(root, txRuleEntry,
                ...
                auto assetRuleId = manageAccountRuleTestHelper
                                       .applyTx(root, assetRuleEntry,
                                                ManageAccountRuleAction::CREATE)
                                       .success()
                                       .ruleID;
                ...
                reviewableRequestResource.reviewableRequest()
                    .details.requestType(ReviewableRequestType::ANY);
                auto revReqRuleEntry =
                    manageAccountRuleTestHelper.createAccountRuleEntry(
                        0, reviewableRequestResource, AccountRuleAction::CREATE,
                        false);
                auto revReqRuleId =
                    manageAccountRuleTestHelper
                        .applyTx(root, revReqRuleEntry,
                                 ManageAccountRuleAction::CREATE)
                        .success()
                        .ruleID;

                auto createSyndicateRoleOp =
                    manageAccountRoleTestHelper.buildCreateRoleOp(
                        "{}", {assetRuleId, revReqRuleId, txRuleId});
                auto syndicateRoleID = manageAccountRoleTestHelper
                                           .applyTx(root, createSyndicateRoleOp)
                                           .success()
                                           .roleID;

                auto createAccountBuilder =
                    CreateAccountTestBuilder().setSource(root);
                ...
                assetCreationRequest =
                    assetTestHelper.createAssetCreationRequest(
                        assetCode, syndicate.key.getPublicKey(), "{}",
                        maxIssuanceAmount, 0, nullptr, preIssuedAmount,
                        testSet.trailingDigitsCount);
                assetTestHelper.createApproveRequest(root, syndicate,
                                                     assetCreationRequest);

                auto saleRequest = SaleRequestHelper::createSaleRequest(
                    assetCode, quoteAsset, currentTime, endTime, softCap,
                    hardCap, "{}",
                    {saleRequestHelper.createSaleQuoteAsset(quoteAsset, price)},
                    requiredBaseAssetForHardCap);
                saleRequestHelper.createApprovedSale(root, syndicate,
                                                     saleRequest);
                auto sales = SaleHelper::Instance()->loadSalesForOwner(
                    syndicate.key.getPublicKey(), testManager->getDB());
                REQUIRE(sales.size() == 1);
                ...
                SECTION("Set tasks without permission")
                {
                    ...
                    manageSaleTestHelper.applyManageSaleTx(
                        syndicate, saleId, data,
                        ManageSaleResultCode::SUCCESS, // No check of inner
                                                       // permission
                ...
                SECTION("Without setting tasks")
                {
                    ...
                    manageSaleTestHelper.applyManageSaleTx(
                        syndicate, saleId, data, ManageSaleResultCode::SUCCESS);
                }
            }
        }
    }
    ...
    SECTION("Participation")
    {
        ...
        createAccountTestHelper.applyCreateAccountTx(
            root, owner.key.getPublicKey(), 1);

        // create base asset
        const AssetCode baseAsset = "ETH";
        uint64_t maxIssuanceAmount = 10 * ONE;
        auto baseAssetRequest = assetTestHelper.createAssetCreationRequest(
            baseAsset, owner.key.getPublicKey(), "{}", maxIssuanceAmount, 0,
            nullptr, maxIssuanceAmount);
        ...
        auto manageBalanceRes =
            ManageBalanceTestHelper(testManager)
                .applyManageBalanceTx(participant, participantID, baseAsset);
        BalanceID baseBalance = manageBalanceRes.success().balanceID;
        BalanceID quoteBalance =
            BalanceHelperLegacy::Instance()
                ->loadBalance(participantID, quoteAsset, db, nullptr)
                ->getBalanceID();

        // pre-issue quote amount
        const uint64_t quotePreIssued = maxNonDividedAmount - precision;
        issuanceHelper.authorizePreIssuedAmount(root, root.key, quoteAsset,
                                                quotePreIssued, root);
        ...
        SECTION("malformed manage offer")
        {
            ...
            uint64_t startTime =
                testManager->getLedgerManager().getCloseTime() + 1000;
            uint64_t endTime = startTime + 1000;
            uint64_t price = 2 * ONE;
            int64_t hardCap =
                bigDivide(maxIssuanceAmount, price, ONE, ROUND_UP);
            auto saleRequest = saleRequestHelper.createSaleRequest(
                baseAsset, quoteAsset, startTime, endTime, hardCap / 2, hardCap,
                "{}",
                {saleRequestHelper.createSaleQuoteAsset(quoteAsset, price)},
                maxIssuanceAmount);
            saleRequestHelper.createApprovedSale(root, owner, saleRequest);
            auto sales = SaleHelper::Instance()->loadSalesForOwner(
                owner.key.getPublicKey(), db);
            ...
            issuanceHelper.applyCreateIssuanceRequest(
                root, quoteAsset, quoteBalanceAmount, quoteBalance,
                SecretKey::random().getStrKeyPublic(), &issuanceTasks);

            // buy a half of sale in order to keep it active
            int64_t baseAmount =
                bigDivide(saleRequest.hardCap / 2, ONE,
                          saleRequest.quoteAssets[0].price, ROUND_UP);
            auto manageOffer = OfferManager::buildManageOfferOp(
                baseBalance, quoteBalance, true, baseAmount,
                saleRequest.quoteAssets[0].price, 0, 0, saleID);

            SECTION("try to participate in not started sale")
            {
                participateHelper.applyManageOffer(
                    participant, manageOffer,
                    ManageOfferResultCode::SALE_IS_NOT_STARTED_YET);
            }
            ...
            SECTION("successfully create participation then delete it")
            {
                ...
                auto offers = OfferHelper::Instance()->loadOffersWithFilters(
                    baseAsset, quoteAsset, &saleID, nullptr, db);
            ...
            SECTION("try to sell base asset being participant")
            {
                ...
                participateHelper.applyManageOffer(
                    participant, manageOffer, ManageOfferResultCode::MALFORMED);
            }
            SECTION("try to participate with negative amount")
            {
                ...
                participateHelper.applyManageOffer(
                    participant, manageOffer,
                    ManageOfferResultCode::INVALID_AMOUNT);
            }
            SECTION("try to participate with zero price")
            {
                ...
                participateHelper.applyManageOffer(
                    participant, manageOffer,
                    ManageOfferResultCode::PRICE_IS_INVALID);
            }
            SECTION("overflow quote amount")
            {
                manageOffer.amount =
                    bigDivide(INT64_MAX, ONE, manageOffer.price, ROUND_UP) + 1;
                participateHelper.applyManageOffer(
                    participant, manageOffer,
                    ManageOfferResultCode::OFFER_OVERFLOW);
            }
            ...
            SECTION("base and quote balances are in the same asset")
            {
                ...
                auto opRes = ManageBalanceTestHelper(testManager)
                                 .applyManageBalanceTx(
                                     participant, participantID, baseAsset);
            ...
            SECTION("price doesn't match sales price")
            {
                ...
                participateHelper.applyManageOffer(
                    participant, manageOffer,
                    ManageOfferResultCode::PRICE_DOES_NOT_MATCH);
            }
            SECTION("try to participate in own sale")
            {
                ...
                auto quoteBalanceID =
                    BalanceHelperLegacy::Instance()
                        ->loadBalance(owner.key.getPublicKey(), quoteAsset, db,
                                      nullptr)
                        ->getBalanceID();
                AccountID ownerID = owner.key.getPublicKey();
                auto baseBalanceID =
                    ManageBalanceTestHelper(testManager)
                        .applyManageBalanceTx(owner, ownerID, baseAsset)
                        .success()
                        .balanceID;
                ...
                participateHelper.applyManageOffer(
                    owner, manageOffer,
                    ManageOfferResultCode::CANT_PARTICIPATE_OWN_SALE);
            }
            SECTION("amount exceeds hard cap")
            {
                // fund account
                issuanceHelper.applyCreateIssuanceRequest(
                    root, quoteAsset, 2 * ONE, quoteBalance,
                    SecretKey::random().getStrKeyPublic(), &issuanceTasks);
                SECTION("by more than ONE")
                {
                    int64_t baseAssetAmount =
                        bigDivide(hardCap + 2 * ONE, ONE, price, ROUND_DOWN);
            ...
            notVerifiedID, AccountType::NOT_VERIFIED);

                // create base balance
                auto baseBalanceID =
            ManageBalanceTestHelper(testManager).applyManageBalanceTx(notVerified,
            notVerifiedID, baseAsset).success().balanceID;

                // fund with quote asset
                auto quoteBalanceID =
            BalanceHelperLegacy::Instance()->loadBalance(notVerifiedID,
            quoteAsset, db, nullptr)->getBalanceID();
                issuanceHelper.applyCreateIssuanceRequest(root, quoteAsset,
            quoteBalanceAmount, quoteBalanceID,
            ...
            &issuanceTasks);
                ...
                participateHelper.applyManageOffer(notVerified, manageOffer,
            ManageOfferResultCode::REQUIRES_KYC);
            }*/
            SECTION("delete participation")
            {
                ...
                SECTION("try to delete offer from closed sale")
                {
                    ...
                    int64_t baseHardCap =
                        bigDivide(hardCap, ONE, price, ROUND_DOWN);
                    manageOffer.amount = baseHardCap - initialAmount;
                    manageOffer.offerID = 0;
                    participateHelper.applyManageOffer(participant,
                                                       manageOffer);
                    ...
                    participateHelper.applyManageOffer(
                        participant, manageOffer,
                        ManageOfferResultCode::SALE_IS_NOT_ACTIVE);
                }
            }
            SECTION("try to participate after end time")
            {
                ...
                participateHelper.applyManageOffer(
                    participant, manageOffer,
                    ManageOfferResultCode::SALE_ALREADY_ENDED);
            }
            SECTION("Invalid offer amount precision")
            {
                if (testSet.baseAsset == "USDN")
                {
                    ...
                    participateHelper.applyManageOffer(
                        participant, manageOffer,
                        ManageOfferResultCode::INCORRECT_AMOUNT_PRECISION);
                }
            }
        }
        ...
        SECTION("invest fee")
        {
            auto offerFee = setFeesTestHelper.createFeeEntry(
                FeeType::OFFER_FEE, quoteAsset, 0, 2 * ONE, nullptr, nullptr,
                FeeFrame::SUBTYPE_ANY, 0, maxNonDividedAmount);
            setFeesTestHelper.applySetFeesTx(participant, &offerFee, false);
            auto fee = setFeesTestHelper.createFeeEntry(
                FeeType::INVEST_FEE, quoteAsset, 0, 1 * ONE, nullptr, nullptr,
                FeeFrame::SUBTYPE_ANY, 0, maxNonDividedAmount);
            ...
            auto feeAssetFrame =
                AssetHelperLegacy::Instance()->mustLoadAsset(quoteAsset, db);
            const uint64_t feeAssetPrecision =
                feeAssetFrame->getMinimumAmount();
            auto manageOfferFee = bigDivide(price, 1 * ONE, 100 * ONE, ROUND_UP,
                                            feeAssetPrecision);
            auto manageOffer = OfferManager::buildManageOfferOp(
                baseBalance, quoteBalance, true, ONE,
        ...
        SECTION("With tasks")
        {
            ...
            auto requestCreationResult =
                saleRequestHelper.applyCreateSaleRequest(
                    syndicate, 0, saleRequest, &allTasks);
            ...
            SECTION("Approve")
            {
                ...
                auto reviewSaleRequestResult =
                    saleReviewer.applyReviewRequestTxWithTasks(
                ...
                REQUIRE(reviewSaleRequestResult.success()
                            .typeExt.saleExtended()
                            .saleID != 0);
                SECTION("Try to remove asset")
                {
                    assetTestHelper.applyRemoveAssetTx(
                        root, testSet.quoteAsset, nullptr,
                        RemoveAssetResultCode::HAS_ACTIVE_SALES);
                }
            }
            ...
            SECTION("Delete asset & approve")
            {
                assetTestHelper.applyRemoveAssetTx(root, testSet.quoteAsset,
                                                   nullptr);
                uint32_t toAdd = 0, toRemove = 1;
                auto reviewSaleRequestResult =
                    saleReviewer.applyReviewRequestTxWithTasks(
                        root, requestID, ReviewRequestOpAction::APPROVE, "",
                        ReviewRequestResultCode::ASSET_PAIR_NOT_FOUND, &toAdd,
                        &toRemove);
            }
        }
    }
