...
{
    auto& balanceHelper = storageHelper.getBalanceHelper();
    auto senderBalanceFrame = balanceHelper.mustLoadBalance(mCreateRedemptionRequest.redemptionRequest.sourceBalanceID);
...
CreateRedemptionRequestOpFrame::CreateRedemptionRequestOpFrame(Operation const& op,
        ...
        , mCreateRedemptionRequest(op.body.createRedemptionRequestOp()) {}
...
//  hope im right
bool
CreateRedemptionRequestOpFrame::doApply(Application & app,StorageHelper & storageHelper, LedgerManager & ledgerManager)
{
    auto& delta = storageHelper.mustGetLedgerDelta();

    auto& balanceHelper = storageHelper.getBalanceHelper();
    auto sourceBalance = balanceHelper.mustLoadBalance(mCreateRedemptionRequest.redemptionRequest.sourceBalanceID);
    if (!sourceBalance) {
    ...
    auto destinationBalance = balanceManager.loadOrCreateBalance(mCreateRedemptionRequest.redemptionRequest.destination,
    ...
    const bool isReferenceExists = requestHelper.isReferenceExist(getSourceID(), mCreateRedemptionRequest.reference, 0);
    ...
    uint32_t allTasks;
    if (!kvHelper.loadTasks(allTasks,makeTasksKeyVector(sourceBalance->getAsset()), mCreateRedemptionRequest.allTasks.get()))
    {
