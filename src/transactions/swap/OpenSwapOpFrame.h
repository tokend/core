...
class OpenSwapOpFrame : public OperationFrame
{
    ...
    bool isSwapAllowed(StorageHelper &sh, BalanceFrame::pointer from, BalanceFrame::pointer to);

    Fee getActualFee(AccountFrame::pointer accountFrame, AssetCode const &transferAsset, uint64_t amount,
                           PaymentFeeType feeType, StorageHelper &sh, LedgerManager& lm);

    bool checkFee(AccountManager& accountManager,
                            AccountFrame::pointer payer,
                            BalanceFrame::pointer chargeFrom,
                            Fee expectedFee, Fee actualFee,
                            AccountID const& commissionID,
                            StorageHelper& sh);
...
public:
    OpenSwapOpFrame(Operation const &op, OperationResult &res, TransactionFrame &parentTx);

    bool doApply(Application &app, StorageHelper& sh, LedgerManager & lm) override;

    bool doCheckValid(Application &app) override;

    static OpenSwapResultCode getInnerCode(OperationResult const &res) {
        return res.tr().openSwapResult().code();
    }

    std::string getInnerResultCodeAsStr() override {
        return xdr::xdr_traits<OpenSwapResultCode>::enum_name(innerResult().code());
    }
};
}