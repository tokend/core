                                   ...
                                   StorageHelper& storageHelper)
{
    if (!condition.account)
    {
        throw std::runtime_error(
            "Expected account ro exists on account rule verifying");
    }

    auto& accountRoleHelper = storageHelper.getAccountRoleHelper();
    auto& accountRuleHelper = storageHelper.getAccountRuleHelper();

    auto accountRoleFrame =
        accountRoleHelper.loadAccountRole(condition.account->getAccountRole());
    if (!accountRoleFrame)
    {
        ...
        throw std::runtime_error(
            "Unexpected state. Expected account role entry to exists.");
    }

    auto accountRuleFrames =
        accountRuleHelper.loadAccountRules(accountRoleFrame->getRuleIDs());

    bool isAllowed = false;

    for (auto accountRuleFrame : accountRuleFrames)
    {
        bool resourceMatches = isResourceMatches(
            condition.resource, accountRuleFrame->getResource());
        ...
        bool actionMatches = isAccountRuleActionMatches(
...
{
    return (conditionAction == actualAction) ||
           (actualAction == AccountRuleAction::ANY);
}
    ...
    AccountRuleResource const actualResource)
{
    ...
    case LedgerEntryType::ACCOUNT_SPECIFIC_RULE:
    {
        ...
        {
            if (conditionResource.accountSpecificRuleExt().v() !=
                actualResource.accountSpecificRuleExt().v())
            {
                return false;
            }

            if (conditionResource.accountSpecificRuleExt()
                    .accountSpecificRule()
            ...
            auto conditionLedgerKey = conditionResource.accountSpecificRuleExt()
                                          .accountSpecificRule()
                                          .ledgerKey;
    ...
    case LedgerEntryType::DATA:
        return isTypeMatches(conditionResource.data().type,
                             actualResource.data().type);
    case LedgerEntryType::LIQUIDITY_POOL:
        isTypeMatches(conditionResource.liquidityPool().firstAssetType,
                      actualResource.liquidityPool().firstAssetType) &&
        isStringMatches(conditionResource.liquidityPool().firstAsset,
                        actualResource.liquidityPool().firstAsset) &&
        isTypeMatches(conditionResource.liquidityPool().secondAssetType,
                      actualResource.liquidityPool().secondAssetType) &&
        isStringMatches(conditionResource.liquidityPool().secondAsset,
