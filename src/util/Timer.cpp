...
tm
VirtualClock::tm_from_time_t(std::time_t timet)
{
    time_point tp = from_time_t(timet);
    return pointToTm(tp);
}

int VirtualClock::getDaysPassed(tm& from, tm& to)
{
	if (from.tm_year == to.tm_year)
	{
		if (to.tm_yday < from.tm_yday)
			throw std::runtime_error("Unexpected order of dates for getDaysPassed");

		return to.tm_yday - from.tm_yday;
	}
	
	if (to.tm_year < from.tm_year)
		throw std::runtime_error("Unexpected order of dates for getDaysPassed");

	auto daysOfFirstYear = getDaysOfYear(from);
	auto daysPassed = daysOfFirstYear - from.tm_yday + to.tm_yday;
	return daysPassed;
}
...
int
VirtualClock::getDaysOfYear(tm& tm)
{
    tm.tm_mday = 31;
    tm.tm_mon = 11;

    VirtualClock::time_point tp = VirtualClock::tmToPoint(tm);
    auto tmRestored = VirtualClock::pointToTm(tp);
    return tmRestored.tm_yday;
}
