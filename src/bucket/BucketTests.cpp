...
TEST_CASE("merging bucket entries", "[bucket][entries]")
{
    ...
    SECTION("dead entry annihilates live entry")
    {
		for (int i = 0; i < 1000; i++)
		{
			auto liveEntry = LedgerTestUtils::generateValidLedgerEntry();
			LedgerKey deadEntry = LedgerEntryKey(liveEntry);
			std::vector<LedgerEntry> live{ liveEntry };
			std::vector<LedgerKey> dead{ deadEntry };
			std::shared_ptr<Bucket> b1 =
				Bucket::fresh(app->getBucketManager(), live, dead);
			REQUIRE(countEntries(b1) == 1);
		}
    }

    SECTION("random dead entries annihilates live entries")
    {
        std::vector<LedgerEntry> live(1000);
        std::vector<LedgerKey> dead;
        for (auto& e : live)
        {
            e = LedgerTestUtils::generateValidLedgerEntry(100);
            if (flip())
            {
    ...
    SECTION("random live entries overwrite live entries in any order")
    {
        ...
        REQUIRE(countEntries(b3) == liveCount);
    }
}
...
TEST_CASE("checkdb succeeding", "[bucket][checkdb]")
{
    ...
    SECTION("failing checkdb")
    {
        ...
        app->getDatabase().getSession()
            << ("UPDATE balance SET amount = (amount + 1) * 2"
                " WHERE account_id = (SELECT accountid FROM accounts LIMIT 1);");
